...
%Predictability
predict = median(data_predictability,1); %Calculate averages
...
pf_totaltimes = sum(data_errors(:,1));
pf_percent = (sum(data_errors(:,2)))/n;

r1 = sum(data_errors(:,3));
r2 = sum(data_errors(:,4));
...
stats_errors = struct('pf_totaltimes',pf_totaltimes,'pf_percent',pf_percent,'r1',r1,'r2',r2);
stats_moments = struct('ER_mean',returns(1),'PD_mean',pdratio(1),'PD_var',pdratio(3),'C_var',macro(2),'D_var',macro(5));
