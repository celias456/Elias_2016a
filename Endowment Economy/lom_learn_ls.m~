function [lom,pf,count,Rtest1,Rtest2] = lom_learn_ls(a,b,ro,gamma,P,Mbar1,Mbar2,Ptm2,Ztm2,shock,mu,T,eshock,eshock_wn,constant_2) 
...
for t = 3:(T+2)
   ...
   Phat2_tm1 = PhiC2_tm1 + Phi2_tm1*P_tm1; %Agent 2 most recent predicted value for equity price
   ...
   R1_t = R1_tm1 + gain * ( ([1;D_tm1] * [1,D_tm1]) - R1_tm1); 
   R1inv = (R1_t)^-1;
   %Update the coefficient matrix - Agent 1
   Phi_t = Phi_tm1 + gain * R1inv * ([1;D_tm1]*(prederror1)); 
   ...
   if constant_2 == 1
       ...
       R2inv = (R2_t)^-1;
       %Update the coefficient matrix - Agent 2
       Theta_t(2,1) = Theta_tm1(2,1) + gain * R2inv * (P_tm2*(prederror2));
   end
