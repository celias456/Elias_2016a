function [lom,pf1,pf2,count1,count2,Rtest1,Rtest2,Phi,Theta] = lom_learn_cg(a1,a2,b1,ro,gamma,P,Mbar1,Mbar2,Ptm2,Ztm2,shock,mu,T,gain,eshock,eshock_wn,constant_2) 
% Returns Law of motion for all variables under CG AL
% Returns a Tx12 matrix lom containing the law of motion under CG learning for
% variables
%    low(:,1) = Equity price
%    low(:,2) = Output
%    low(:,3) = Consumption
...
%    lom(:,8) = investment
%    lom(:,9) = gross profits
%    lom(:,10) = previous period price
%    lom(:,11) = Agent 1 previous period prediction of price
%    lom(:,12) = Agent 2 previous period prediction of price
...
for t = 3:(T+2)
   ...
   R1_t = R1_tm1 + gain(1) * ( ([1;P_tm2;Z_tm1] * [1,P_tm2,Z_tm1]) - R1_tm1); 
   R1inv = (R1_t)^-1;
   %Update the coefficient matrix - Agent 1
   Phi_t = Phi_tm1 + gain(1) * R1inv * ([1;P_tm2;Z_tm1]*(prederror1)); 
   ...
   if constant_2 == 1
       ...
       R2inv = (R2_t)^-1;
       %Update the coefficient matrix - Agent 2
       Theta_t(2,1) = theta_tm1 + gain(2) * R2inv * (P_tm2*(prederror2));
   end
